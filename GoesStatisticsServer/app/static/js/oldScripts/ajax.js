...
async function changeTodoState(todoId) {
    ...
    try {
        ...
        // Меняем статус карточки (если карточка цвета info, то текущий статус "выполнена")
        let statusText = todoLi.classList.contains('bg-info') ? 'Статус: Не выполнено' : 'Статус: Выполнено';
    ...
    // Убираем загрузку
    //setLoading(false);
    // Можно при включённой фильтрации в зависимости от того, какое состояние было присвоено карточке, скрывать её
}
