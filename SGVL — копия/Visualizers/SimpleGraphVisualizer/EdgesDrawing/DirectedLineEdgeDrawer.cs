...
namespace SGVL.Visualizers.SimpleGraphVisualizer.EdgesDrawing {
    ...
    class DirectedLineEdgeDrawer : UndirectedLineEdgeDrawer, IEdgeDrawer {
        ...
        private void DrawEdge(Graphics g, Edge edge, bool isEdgeSelected) {
            ...
            g.DrawLine(new Pen(new SolidBrush(color), edgeWidth), sPoint.X, sPoint.Y, tPoint.X, tPoint.Y);
            // Рисуем усики стрелки (от tP к p1 и от tP к p2)
            g.DrawLine(new Pen(new SolidBrush(color), edgeWidth), tPoint.X, tPoint.Y, p1.X, p1.Y);
            g.DrawLine(new Pen(new SolidBrush(color), edgeWidth), tPoint.X, tPoint.Y, p2.X, p2.Y);
